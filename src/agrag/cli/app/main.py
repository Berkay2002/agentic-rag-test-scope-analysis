...
from agrag.config.paths import (
    BM25_INDEX_PATH,
    EVAL_SUITES_PATH,
    MOCK_DATASET_PATH,
    MOCK_EVAL_QUERIES_PATH,
)
...
@click.option(
    "--suite-file",
    type=click.Path(exists=True),
    default=str(EVAL_SUITES_PATH),
    help=f"Path to evaluation suites registry (default: {EVAL_SUITES_PATH})",
)
@click.option(
    "--output",
    type=click.Path(),
...
@click.option(
    "--bm25-index",
    type=click.Path(),
    default=str(BM25_INDEX_PATH),
    help=f"Path to BM25 index file (default: {BM25_INDEX_PATH})",
)
def evaluate(
...
@click.option("--output", default=str(MOCK_DATASET_PATH), help="Output file path")
@click.option(
...
def generate(
    ...
    Examples:
      ...
      agrag generate --with-eval --eval-output data/mock/eval_queries.json
...
@click.option(
    "--dataset",
    type=click.Path(exists=True),
    default=str(MOCK_DATASET_PATH),
    help="Path to synthetic dataset (generated by 'generate' command)",
)
@click.option(
    "--output",
    default=str(MOCK_EVAL_QUERIES_PATH),
    help="Output file for evaluation dataset",
)
...
def ingest(dataset_path: str):
    ...
    Examples:
      agrag ingest data/mock/synthetic_dataset.json
    """
    from agrag.data.ingestion import DataIngestion
